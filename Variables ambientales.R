...
gg          <- ggplot(vls)  +
  ...
  scale_fill_gradientn(colours = colores1, 
                       na.value = 'white', limits = c(0, 876), breaks = seq(0, 876, 50)) +
  ...
  labs(title = 'Precipitación mensual - Madre de Dios', fill = 'mm',  x = 'Longitud', y = 'Latitud', caption = "") +
  theme(
  ...
  ggtitle("Precipitación mensual - Madre de Dios ")+
...
ggsave(plot = gg,"Precipitacion.png", units = "cm", width = 29, #ancho
       height = 29, #Largo
       dpi = 1200)
...
Peru_Depa   <- getData('GADM', country='Peru', level=2) %>%
  st_as_sf() 
Cusco         <-subset(Peru_Depa , NAME_1 == "Madre de Dios") 
...
A=ggplot(vls)  +
  ...
  labs(title = 'Temperatura Promedio mensual ºC- Madre de Dios', fill = 'Temperatura \nPromedio mensual ºC',  x = 'Longitud', y = 'Latitud', caption = "Gorky Florez") +
  theme(
    plot.background = element_rect(fill = "black"),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    ...
    plot.caption = element_text(size = 10, hjust = 0.95, color = "white", family="serif", face = "italic")) +
  guides(shape = guide_legend(override.aes = list(size = 10)))+
  ggtitle("Temperatura Promedio mensual ºC de Madre de Dios")+
  labs(subtitle="Ing. Gorky Florez Castillo", x="Longitud",y="Latitud",
       caption="Fuente: Data: https://www.worldclim.org")+
  ggspatial::annotation_north_arrow(
    location = "tr", which_north = "true",
...
ggsave(plot = A,"Temperatura.png", units = "cm", width = 29, #ancho
       height = 29, #Largo
       dpi = 1200)
